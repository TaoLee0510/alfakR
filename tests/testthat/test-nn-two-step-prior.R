test_that("empirical_two_step prior mode and controls validate", {
  expect_identical(
    alfakR:::validate_nn_prior_mode("empirical_two_step"),
    "empirical_two_step"
  )
  expect_silent(
    alfakR:::validate_nn_prior_controls(
      ...
      nn_two_step_max_weight_ratio = 1,
      nn_two_step_lambda = 1,
      nn_two_step_reuse_sd = NULL,
      nn_two_step_uncertainty_floor = NULL
    )
  )
  expect_error(
    alfakR:::validate_nn_prior_controls(nn_two_step_min_delta_n = 0L),
    "`nn_two_step_min_delta_n`"
  )
  expect_error(
    alfakR:::validate_nn_prior_controls(nn_two_step_min_exposure = -1),
    "`nn_two_step_min_exposure`"
  )
  expect_error(
    alfakR:::validate_nn_prior_controls(nn_two_step_min_observed_count = 1.5),
    "`nn_two_step_min_observed_count`"
  )
  expect_error(
    alfakR:::validate_nn_prior_controls(nn_two_step_max_weight_ratio = -0.1),
    "`nn_two_step_max_weight_ratio`"
  )
  expect_error(
    alfakR:::validate_nn_prior_controls(nn_two_step_lambda = -0.1),
    "`nn_two_step_lambda`"
  )
  expect_error(
    alfakR:::validate_nn_prior_controls(nn_two_step_reuse_sd = -0.1),
    "`nn_two_step_reuse_sd`"
  )
  expect_error(
    alfakR:::validate_nn_prior_controls(nn_two_step_uncertainty_floor = -0.1),
    "`nn_two_step_uncertainty_floor`"
  )
})

test_that("new two-step controls do not affect existing none mode", {
  x <- matrix(
    c(80, 60,
      4, 2,
      0, 3),
  ...
  with_controls <- suppressWarnings(alfakR:::solve_fitness_bootstrap(
    ...
    pm = 1e-4,
    nn_prior = "none",
    nn_two_step_min_delta_n = 9L,
    nn_two_step_min_exposure = 100,
    nn_two_step_min_observed_count = 3L,
    nn_two_step_max_weight_ratio = 0.2,
    nn_two_step_lambda = 0.1,
    nn_two_step_reuse_sd = 10,
    nn_two_step_uncertainty_floor = 5
  ))
...
test_that("empirical_two_step records fallback when no two-step candidates are retained", {
  x <- matrix(
    c(80, 60),
    nrow = 1,
    byrow = TRUE,
  ...
  expect_true("nn_two_step_node_diagnostics" %in% names(res))
})

test_that("two-step path responsibilities normalize shared descendants", {
  paths <- data.frame(
  ...
  out <- alfakR:::compute_two_step_path_responsibilities(paths)
...
make_two_step_test_context <- function() {
  list(
    child = list(
      ni = "2.2.1",
      nj = "2.2.2",
...
test_that("two-step backward correction changes a one-step estimate in the outward direction", {
  contexts <- make_two_step_test_context()
  ...
  corrected <- alfakR:::apply_two_step_backward_correction(
...
test_that("larger two-step reuse uncertainty weakens the outward correction", {
  contexts <- make_two_step_test_context()
  outward_paths <- data.frame(
    one_step = "child",
    descendant = "2.2.0",
  ...
  low_reuse <- alfakR:::apply_two_step_backward_correction(
    nn_child_contexts = contexts,
    f1_initial = c(child = 0),
    outward_paths = outward_paths,
    mu01 = 0,
  ...
  high_reuse <- alfakR:::apply_two_step_backward_correction(
    nn_child_contexts = contexts,
    f1_initial = c(child = 0),
    outward_paths = outward_paths,
    mu01 = 0,
