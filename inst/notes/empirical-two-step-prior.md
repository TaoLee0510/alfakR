# Empirical Two-Step Nearest-Neighbour Prior

`nn_prior = "empirical_two_step"` is an opt-in correction for sparse
...
2. Builds two-step candidates reachable from provisional one-step nodes.
   ...
   `nn_two_step_min_observed_count`; unobserved candidates must pass
   `nn_two_step_min_exposure`, or an adaptive projected-exposure threshold
...
4. Learns separate 0->1 and 1->2 Gaussian delta priors with
   `nn_prior_sd_floor` applied to both step-specific standard deviations.
...
fitness variance, and `nn_two_step_reuse_sd` because the two-step estimate
...
`nn_two_step_save_diagnostics = TRUE`; the same diagnostics are also carried
